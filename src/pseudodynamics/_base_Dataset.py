...
class AnnDataset(Dataset):
    ...
    def __init__(
        ...
        assert (
            cellstate_key in AnnData.obsm.keys()
        ), f"cellstate key `{cellstate_key}` not found in adata"
        assert (
            timepoint_key in AnnData.obs.keys()
        ), f"timepoint key `{timepoint_key}` not found in adata"
        ...
        self.popD["std"] = self.popD["std"] / N0
...
class MeshGrid(Dataset):
    ...
    def compute_grid_density(self):
        ...
        for tb_idx, t_b in enumerate(self.popD["t"]):
            ...
            var_ls.append(self.popD["std"][tb_idx] ** 2 / n_exp)
