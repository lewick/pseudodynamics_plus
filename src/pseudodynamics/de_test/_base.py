...
def fitGAM(count_matrix, cell_time, n_knots=7, cell_weights=None, gene_symbol=None):
    ...
    design_matrix = np.repeat(
        design_matrix[np.newaxis, :], count_matrix.shape[1], axis=0
    )  # shape (genes, timepoints)
    ...
    for i_gene in range(count_matrix.shape[1]):
        ...
        if np.all(expr == expr[0]) or len(np.unique(cell_time)) < 2:
            ...
            pred_func = interp1d(
                cell_time, np.zeros_like(cell_time), fill_value="extrapolate"
            )
            models[i_gene] = {
                "model": None,
        ...
        try:
            gam = GLMGam(
                endog=expr, smoother=bs, alpha=1e-6, family=NegativeBinomial(alpha=1.0)
...
def load_gamfit(save_dir):
    ...
    for i, gene_id in enumerate(gam_fit["gene_names"]):
        ...
        if os.path.exists(pkl_path):
            ...
            models[gene_id]["pred_func"] = interp1d(
                cell_time, res.predict(), fill_value="extrapolate"
            )
        else:
...
def process_gene_chunk(
    gene_indices, expression_matrix, cell_time, n_knots, gene_symbol
):
    """
...
def _validate_shapes(beta, sigma, L, context=""):
    ...
    if beta.shape[0] != sigma.shape[0]:
        raise ValueError(
            f"Beta and sigma dimension mismatch: beta={beta.shape}, sigma={sigma.shape}"
        )

    if L.shape[1] != beta.shape[0]:
        raise ValueError(
            f"L matrix and beta dimension mismatch: L={L.shape}, beta={beta.shape}"
        )
            ...
            try:
                ...
                if pairwise_test:
                    for pred_diff, lp_diff, (lineage_a, lineage_b) in zip(
                        ...
                        wald_stat, df, p_value = _wald_test(pred_diff, lp_diff, cov)
                ...
                if global_test and len(pred_diffs) > 0:
                    ...
                    result["globally"][var_name] = (
                        ...
                        df,
                        p_value,
                        np.mean(np.concatenate(pred_diffs)),
                    )
