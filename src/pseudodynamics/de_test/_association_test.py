...
class AssociationTest:
    ...
    def association_test(
        ...
        if restrcited_pseudotime is None:
            ...
            assert (
                len(restrcited_pseudotime) == 2
            ), "please pass a list of [min_pdt, max_pdt]"
        ...
        for gene_idx, model_info in models.items():
            ...
            try:
                ...
                L = np.zeros(
                    (n_points - 1, X_points.shape[1])
                )  # Use X_points.shape[1] for correct dimensions
                ...
                L_reduced = np.broadcast_to(
                    L_reduced, (L_reduced.shape[0], beta.shape[0])
                )
                ...
                if inverse == "chol":
                    try:
                        ...
                        inv_cov = solve_triangular(
                            L_chol,
                            solve_triangular(
                                L_chol, np.eye(L_chol.shape[0]), lower=True, trans="T"
                            ),
                            lower=True,
                        )
                    except np.linalg.LinAlgError:
                        logger.warning(
                            "Cholesky decomposition failed, using pseudo-inverse"
                        )
                    ...
                    try:
                        ...
                        logger.warning(
                            "Eigen decomposition failed, using pseudo-inverse"
                        )
                        inv_cov = np.linalg.pinv(cov_contrast)
                ...
                try:
                    ...
                    logger.error(
                        f"est_reduced shape: {est_reduced.shape}, inv_cov shape: {inv_cov.shape}"
                    )
                    raise
                ...
                if model_info["model"]:
                    try:
                        ...
                        filtered_models[gene_idx] = {
                            "model": res,
                            "pred_func": interp1d(
                                cell_time, res.predict(), fill_value="extrapolate"
                            ),
                            "expr": expr,
                        }
                    except:
...
def _run_test_chunk(gene_indices, gam_fit, pseudotime_range):
    ...
    # Filter genes
    filtered_models = {idx: gam_fit["models"][idx] for idx in gene_indices}
    ...
    test = AssociationTest(filtered_gam_fit, ["lineage"])
...
def run_association_test_parallel(gam_fit, pseudotime_range, chunk_size=10, n_cores=20):
    ...
    gene_indices = np.array_split(
        np.arange(n_genes), chunk_size * n_cores
    )  # One gene per chunk

    # Create partial function with fixed parameters
    worker_func = partial(
        _run_test_chunk, gam_fit=gam_fit, pseudotime_range=pseudotime_range
    )
