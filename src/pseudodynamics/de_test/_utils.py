...
def select_top_de_genes(results_df, qval_threshold=0.05):
    ...
    significant["fdr"] = multitest.multipletests(
        significant["pvalue"], method="fdr_bh"
    )[1]

    # Sort by Wald statistic
    significant = significant.query("`fdr` <= @qval_threshold").sort_values(
        "waldStat", ascending=False
    )
