...
class ExperimentConfig:
    ...
    def __init__(
        ...
        if (args is not None) and (model is not None):
            ...
            try:
                ...
                pass
            self.from_json(config, main_dir=main_dir)
            ...
            raise ValueError(
                f"{config} config file not Found, args and model can not be empty for new experiment"
            )
        ...
        if main_dir is not None:
            try:
                for key in ["checkpoint_dir", "save_dir"]:
                    if (
                        key in self.experiment_config
                        and "logs/" in self.experiment_config[key]
                    ):
                        old_main = self.experiment_config[key].split("logs/")[0]
                        if old_main != "":
                            self.experiment_config[key] = self.experiment_config[
                                key
                            ].replace(old_main, main_dir)
                        else:
                            ...
                            self.experiment_config[key] = os.path.join(
                                main_dir, self.experiment_config[key]
                            )
            except:
                pass
    ...
    def find_lastest_ckpt(self):
        ...
        loss = [
            float(
                re.match(
                    r"epoch=\d{1,3}-\w{3,10}_loss=([-, \.,\d]{1,30}).ckpt", ckpt
                ).group(1)
            )
            for ckpt in ckpts
        ]
