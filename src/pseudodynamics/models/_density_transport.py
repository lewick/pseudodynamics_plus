...
class Density_Transfer(nn.Module):
    ...
    def __init__(
        self, pde_model=None, stochastic=False, noise_schedule=None, n_repeat=30
    ):
        super().__init__()
    ...
    def velocity(self, t, s):
        ...
        ncell = s.shape[0]
        t_in = (
            torch.full((s.shape[0], 1), t.item()).to(device)
            * self.model.time_scale_factor
        )

        if self.stochastic:
            noise = torch.broadcast_to(
                torch.sqrt(self.model.D(s, t_in) * 2).view(ncell, -1), s.shape
            )
            ds = self.model.v(s, t_in) + self.noise_schedule(
                s, t_in
            ) * noise * torch.randn((ncell, 1)).to(device)
        else:
        ...
        with torch.no_grad():
            try:
                s_out = odeint(
                    self.velocity,
                    y0=s0,
                    t=torch.tensor(integrate_time).to(device)
                    * self.model.time_scale_factor,
    ...
    def transition_by_batch(self, s0, u0, integrate_time, n_interval=10, ncell=200):
        ...
        for ic in range(0, s0.shape[0], ncell * 4):
            ...
            u0_by_time.append(
                self.relu(intout[0]).detach().cpu().numpy()
            )  # (time, chunk)
            del intout
        ...
        for ic in range(0, s0.shape[0], ncell):
            ...
            for offset in range(n_interval):  # the ith time of transition
                ...
                s_traj = (
                    torch.from_numpy(s_traj_ay[:, ic : ic + ncell, :])
                    .float()
                    .to(device)
                )
                ...
                for i in range(n_interval - offset):
                    ...
                    left_u = (
                        torch.from_numpy(leftover_u_global[-1][i])
                        .requires_grad_()
                        .to(device)
                    )  # left from last round of diff trajactory
                    inflow_u = (
                        torch.from_numpy(diff_flow_u_local[-1])
                        .requires_grad_()
                        .to(device)
                    )  # inflow from last step of the same round of diff trajectory
                    ...
                    t = tn1 + delta_t
                    s_last, u_stay, s_next, u_flow = odeint_adjoint(
                        self,
                    ...
                    with torch.no_grad():
                        diff_flow_u_local.append(
                            self.relu(u_flow[-1]).detach().to("cpu").numpy()
                        )
                        leftover_u_local.append(
                            self.relu(u_stay[-1]).detach().to("cpu").numpy()
                        )
                        tn1 = t
                ...
                diff_flow_u_local = np.stack(diff_flow_u_local)[
                    1:
                ]  # length : 100 - offset
                leftover_u_local = np.stack(
                    leftover_u_local
                )  # [1:]   # length : 100 - offset
...
class DT_analysis:
    ...
    def load_result(self, result_dir):
        ...
        for files in os.listdir(self.result_dir):
            ...
            if files.endswith("cellbarcode.npy"):
                self.cb_dict[day] = np.load(
                    os.path.join(self.result_dir, files), allow_pickle=True
                )
                ...
                self.trajectory_dict[day] = np.load(
                    os.path.join(self.result_dir, files)
                )
    ...
    def annotate_trajectory(self, cellstate_key, obs_key, copy=False):
        ...
        for t, traj in self.trajectory_dict.items():
            ...
            for step in range(1, traj.shape[0]):
                ...
                annotations = efun.assign_nearest_cell(
                    traj[step],
                    self.adata,
                    cellstate_key=cellstate_key,
                    annotation=obs_key,
                )
                proportions = self.summarize_cell_proportions(
                    annotations, celltype_list
                )
    ...
    def density_by_celltype_step(self, celltypes, step=-1, norm=True):
        ...
        for time in DT_dict:
            ...
            assert (
                DT_M.flatten().shape[0] == ct_day_df.shape[0]
            ), "The number of cell types and the number of rows in the DT matrix should be equal"

            ct_day_df.loc[:, celltypes] = ct_day_df.loc[
                :, celltypes
            ] * DT_M.flatten().reshape(-1, 1)
            ct_day_df["Day"] = time
        ...
        self.ct_density["start_day"] = (
            self.ct_density["Day"].apply(lambda x: x.split("-")[0]).astype(int)
        )
    ...
    def heatmap_cell_proportions(
        self,
        celltypes,
        value="assignment_probability",
        step=-1,
        paletter=None,
        log_normalize=False,
        **kwargs,
        ...
        **kwargs: kwargs for seaborn clustermap
        ...
        ax = g.ax_row_colors
        g.ax_cbar.set_title(["", "log "][log_normalize] + value)
    ...
    def hierarchical_clustering(
        ...
        if density_matrix is None:
            ...
            raise ValueError(
                "no input given !!! provide density matrix or value_source and celltypes"
            )
    ...
    def clustermap(
        ...
        **kwargs,
        ...
        g.ax_cbar.set_title(["", "log "][log_normalize] + "density")
