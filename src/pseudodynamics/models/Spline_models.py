...
class MultiDim_CubicSpline(nn.Module):
    def __init__(self, y, x=None, n_knot=None, collapse=False):
        ...
        assert (
            len(y.shape) == 2
        ), "for MultiDim CubicSpline the initital y must be 2-d shape : [#knots, #dimensions]"
        assert (
            y.shape[0] == x.shape[0]
        ), "the number of anchor knots is not consistent between x and y"
    ...
    def forward(self, xs, t=None) -> torch.Tensor:
        ...
        assert (
            len(xs.shape) == 2
        ), "Input xs must be 2-dim shape : [#points, #dimensions]"
...
class Cspline_PINN(PINN_base_sim):
    def __init__(
        self, *, n_knot=9, n_dim=1, D_collapse=False, v_collapse=False, **kwargs
    ):
        """
        ...
        if n_dim == 1:
            ...
            self.D = MultiDim_CubicSpline(
                y=torch.ones((n_knot, n_dim)).float(),
                n_knot=n_knot,
                collapse=D_collapse,
            )
            self.v = MultiDim_CubicSpline(y=vy, n_knot=n_knot, collapse=v_collapse)
            self.g = MultiDim_CubicSpline(
                y=torch.ones((n_knot, n_dim)).float(), n_knot=n_knot, collapse=True
            )
