...
class pde_params_base(pl.LightningModule):
    ...
    def configure_optimizers(self):
        ...
        if self.schedule_lr == "CosineAnnealingLR":
            scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(
                optimizer, T_max=self.max_epochs, eta_min=1e-6
            )
        elif self.schedule_lr == "CyclicLR":
            scheduler = torch.optim.lr_scheduler.CyclicLR(
                optimizer,
                base_lr=self.lr * 0.1,
                max_lr=self.lr,
                step_size_up=self.max_epochs // 10,
                cycle_momentum=False,
            )
        elif self.schedule_lr == "StepLR":
            scheduler = torch.optim.lr_scheduler.StepLR(
                optimizer, step_size=self.max_epochs // 4, gamma=0.1
            )
        elif self.schedule_lr == "ExponentialLR":
    ...
    def trace_div(self, f, s):
        ...
        for i in range(s.shape[1]):
            sum_diag += (
                torch.autograd.grad(
                    f[:, i].sum(), s, create_graph=True, allow_unused=True
                )[0]
                .contiguous()[:, i]
                .contiguous()
            )
    ...
    def gradient_of(self, out, variable):
        return torch.autograd.grad(out, variable, create_graph=True, allow_unused=True)[
            0
        ]
    ...
    def predict_nabla_v(self, train_DS, device=None):
        ...
        v_pred_ay = (
            np.concatenate(v_ls, axis=0).reshape(n_timepoint, -1)
    ...
    def predict_param(self, train_DS, device=None):
        ...
        v_pred_ay = (
            np.concatenate(v_ls, axis=0).reshape(n_timepoint, -1, v_dim)
            / self.time_scale_factor
        )
        g_pred_ay = (
            np.concatenate(g_ls, axis=0).reshape(n_timepoint, -1)
            / self.time_scale_factor
        )
        D_pred_ay = (
            np.concatenate(D_ls, axis=0).reshape(n_timepoint, -1, D_dim).squeeze()
            / self.time_scale_factor
        )
    ...
    def density_transfer(self, t, states):
        ...
        with torch.set_grad_enabled(True):
            ...
            global_drift = torch.div(
                torch.broadcast_to(vu_next - vu, s_t.shape), s_next - s_t
            )  # .sum(dim=1)
...
class pde_params(pde_params_base):
    ...
    def __init__(
        ...
        self.D_penalty = 0.1 if D_penalty is None else D_penalty
        self.eps = 1e-10  # Central epsilon for numerical stability
        ...
        self.u = MLP_surrogate(
            channels=channels, activation_fn=activation_fn, time_sensitive=True
        )
        ...
        if v_channels is None:
            v_channels = (
                channels[:-1] + [1] if collapse_v else channels[:-1] + [self.n_dim]
            )
        self.v = MLP_Module(
            channels=v_channels,
            activation_fn=activation_fn,
        ...
        if D_channels is None:
            D_channels = (
                channels[:-1] + [1] if collapse_D else channels[:-1] + [self.n_dim]
            )
        self.D = MLP_Module(
            channels=D_channels,
            activation_fn=activation_fn,
    ...
    def ode_func(self, t, states):
        ...
        device = s.device
        t_in = (
            torch.full((s.shape[0], 1), t.item() * self.time_scale_factor)
            .float()
            .to(device)
        )
    ...
    def forward_density_loss(self, s, t, ut):
        ...
        log_density_loss_t = self.loss_fn(torch.log(ut + self.eps), log_u_pred)
    ...
    def forward_simulation(self, s, t, tp1, ut):
        ...
        # Convert initial condition to log scale if in log-space mode
        ut_init = torch.log(ut + 1e-10) if self.log_transform else ut
        init_condition = (
            ut_init,
            s,
            duds_init,
        ...
        step_size = 3.0
        # timepoints = torch.arange(t0, t1+step_size, step_size).type(torch.float32).to(device)
        timepoints = torch.tensor([t0, t1]).type(torch.float32).to(device)
        # instead of just the start and end timepoint, also use one in the middle to help the training of the PINN
        # timepoints = torch.tensor([t0, (t0+t1)/2, t1]).type(torch.float32).to(device)
        u_int, s_t, duds, growth, drift, diffuse = odeint_adjoint(
            self,
            y0=init_condition,
            t=timepoints,  # torch.tensor([t0, t1]).type(torch.float32).to(device),
            atol=1e-06,
            rtol=1e-05,
            method="dopri5",
            adjoint_options={"norm": "seminorm"},
        )
        # check for any NaN or inf in u_int
        if torch.isnan(u_int).any() or torch.isinf(u_int).any():
            print("Warning: NaN or Inf detected in u_int during ODE integration.")
            print(
                "u_int stats - min:",
                u_int.min().item(),
                "max:",
        ...
        # Convert back to linear scale if in log-space mode
        if self.log_transform:
            u_int = torch.exp(u_int)
        else:
            u_int = nn.functional.relu(u_int)

        return u_int, s_t, duds, growth, drift, diffuse, timepoints
    ...
    def residual_loss(self, s, t) -> torch.Tensor:
        ...
        if not self.log_transform:
            ...
            # Convert to log-scale to match density loss: d(log_u)/dt = (1/u) * du/dt
            u = self.get_u(s, t)
            # Add small epsilon to avoid division by zero
            u_safe = torch.clamp(u, min=self.eps)
            dudt_log = dudt / u_safe
            rhs_log = rhs / u_safe
        else:
            dudt_log = dudt
            rhs_log = rhs
        return self.loss_fn(rhs_log.squeeze(), dudt_log.squeeze())
        # return
    ...
    def training_step(self, train_batch, index):
        ...
        # loss 2 : dynamics
        u_int, s_t, duds, growth, drift, diffuse, timepoints = self.forward_simulation(
            s, t, tp1, ut
        )
        # check that growth[0] is 0
        # check for NaN or inf in u_int
        if torch.isnan(u_int).any() or torch.isinf(u_int).any():
            print("Warning: NaN or Inf detected in u_int during simulation.")
            print(
                "u_int stats - min:",
                u_int.min().item(),
                "max:",
            ...
            # Optionally, you can set the loss to a high value to avoid training instability
            return torch.tensor(1e6, requires_grad=True).to(s.device)
        if self.log_transform:
            log_sim_loss_tp1 = self.loss_fn(
                torch.log(utp1 + self.eps), torch.log(u_int[-1] + self.eps)
            )
        else:
            log_sim_loss_tp1 = self.loss_fn(
                torch.log(utp1 + self.eps), torch.log(u_int[-1] + self.eps)
        ...
        for tp in timepoints:
            tp = tp.unsqueeze(0).expand(
                s.shape[0]
            )  # tp is shape [], but should be shape (1024,)
            R_loss += self.residual_loss(s, tp) * 1 / len(timepoints)
            D_norm += self.restrict_D(s, t, exp=False) * 1 / len(timepoints)
            V_norm += (
                torch.norm(self.v(s, t), p=2).mean() * 1 / len(timepoints)
            )  # constrain D
        # R_loss = self.residual_loss(s, timepoints)
        ...
        # Note: ut and utp1 are in LINEAR scale (from data batch, not log)
        N_t = ut.sum()
        N_tp1 = utp1.sum()
        # print('utp1 sum', N_tp1.item(), 'u_int sum', u_int[-1].sum().item())
        # For the integral: ∫∫ g(s,t)*u(s,t) ds dt
        growth_integral = torch.zeros((), device=device)

        if self.log_transform:
            # In log mode: recompute instantaneous g values (not accumulated)
            with torch.set_grad_enabled(True):
                for i in range(len(timepoints) - 1):
                    dt = timepoints[i + 1] - timepoints[i]
                    # Compute g at timepoint i
                    s_i = s_t[i].clone().requires_grad_(True)
                    t_i = (
                        timepoints[i]
                        .unsqueeze(0)
                        .expand(s_i.shape[0])
                    ...
                    _, g_i, _, _ = self.equation(s_i, t_i)

                    # Compute g at timepoint i+1
                    s_ip1 = s_t[i + 1].clone().requires_grad_(True)
                    t_ip1 = (
                        timepoints[i + 1]
                        .unsqueeze(0)
                        .expand(s_ip1.shape[0])
                        .unsqueeze(1)
                        .float()
                        .to(device)
                        .requires_grad_(True)
                    )
                    _, g_ip1, _, _ = self.equation(s_ip1, t_ip1)

                    # Trapezoidal rule: average instantaneous g at consecutive timepoints
                    integrand_i = (g_i * u_int[i]).sum()
                    integrand_ip1 = (g_ip1 * u_int[i + 1]).sum()
                    growth_integral += (integrand_i + integrand_ip1) / 2 * dt
        else:
            # In linear mode, `growth` is integrated by the ODE solver already
            # (since `ode_func` returns `growth` as a derivative state).
            growth_integral = growth[-1].sum()

        if self.log_transform:
            # Compare in log scale: log(N_tp1) vs log(N_t) + growth_integral
            # Equation: N_tp1 = N_t * exp(growth_integral)
            log_N_t = torch.log(N_t + self.eps)
            log_N_tp1_plus_growth = log_N_t + growth_integral
            log_deviation = log_N_tp1_plus_growth - torch.log(N_tp1 + self.eps)
            growth_loss = torch.nn.functional.huber_loss(
                torch.zeros_like(log_deviation),
                log_deviation,
                delta=1.0,
                reduction="mean",
            )
            ...
            # Normalized difference: scale-invariant, bounded, consistent throughout training
            growth_loss = torch.abs(mass_gain - predicted_gain) / (
                torch.abs(mass_gain) + torch.abs(predicted_gain) + 1e-10
            )
            # print('mass_gain:', mass_gain.item(), 'predicted_gain:', predicted_gain.item(), 'growth_loss:', growth_loss.item())

        # print('u_int:', u_int[-1].sum().item())
        # print('u_surrogate:', self.u(s, t).exp().sum().item())

        V_norm_weight = 50
        sim_loss_weight = 5
        total_loss = (
            log_density_loss_t
            + log_density_loss_tp1
            + sim_loss_weight * log_sim_loss_tp1
            + self.D_penalty * D_norm
            + V_norm_weight * V_norm
            + self.R_weight * R_loss
            + self.growth_weight * growth_loss
        )
        g_grad_norm = sum(
            p.grad.norm() if p.grad is not None else 0 for p in self.g.parameters()
        )
        # print(f"g gradient norm: {g_grad_norm}")
        ...
        with torch.no_grad():
            ...
            self.log(
                "growth_loss",
                (self.growth_weight * growth_loss).item(),
                on_epoch=True,
                on_step=False,
                prog_bar=True,
            )
            self.log(
                "boundary_loss",
                (log_density_loss_t + log_density_loss_tp1).item(),
                on_epoch=True,
                on_step=False,
                prog_bar=True,
            )
            ...
            self.log(
                "integrat_loss",
                (sim_loss_weight * log_sim_loss_tp1.item()),
                on_epoch=True,
                on_step=False,
                prog_bar=True,
            )
            ...
            self.log(
                "V_penalty",
                (V_norm_weight * V_norm).item(),
                on_epoch=True,
                on_step=False,
                prog_bar=True,
            )
            self.log(
                "total_loss", total_loss, on_epoch=True, on_step=False, prog_bar=True
            )
            # also lr rate
            # lr = self.optimizers().param_groups[0]['lr']
            # print(f"Epoch {self.current_epoch}: {lr}")
            # self.log_dict({'learning_rate': float(f'{lr:.2e}')}, on_epoch=True, prog_bar=True)
...
class pde_params_fastmode(pde_params):
    ...
    def training_step(self, train_batch, index):
        ...
        u_int, s_t, duds, growth, drift, diffuse = self.forward_simulation(
            s, t, tp1, ut
        )

        log_sim_loss_tp1 = self.loss_fn(
            torch.log(utp1 + 1e-10), torch.log(u_int[-1] + 1e-10)
        )

        # loss 3 : constrain related loss : use the original cell state
        t_rand = (
            torch.Tensor(s.shape[0], 1)
            .uniform_(t[0].item(), tp1[0].item())
            .float()
            .to(s.device)
        )
...
class log_pde_params(pde_params):
    ...
    def equation(self, s, t) -> tuple:
        ...
        dlogudt = self.gradient_of(logu.sum(), t)
        # check for nans
        if torch.isnan(dlogudt).any():
            print("Warning: dlogudt contains NaN values.")
        ...
        # Diffusion term: ∇D⋅∇log(u) + D(∇log(u))² + D∇²log(u)
        term1 = self.mul(dDds, dloguds)  # ∇D ⋅ ∇log(u)
        term2 = self.mul(D, self.mul(dloguds, dloguds))  # D * (∇log(u))²
        term3 = self.mul(D, dlogudss)  # D * ∇²log(u)
        diffusion = term1 + term2 + term3

        # drift : ∇v + (∇logu·v)
        drift = dvds.sum(dim=1) + self.mul(dloguds, v)
        growth = g
        # check for nans in growth, drift, diffusion
        if torch.isnan(growth).any():
            print("Warning: growth contains NaN values.")
        if torch.isnan(drift).any():
            print("Warning: drift contains NaN values.")
        if torch.isnan(diffusion).any():
            print("Warning: diffusion contains NaN values.")

        return dlogudt, growth, drift, diffusion
...
class pde_singlebranch_twotimepoints(pde_params_base):
    ...
    def ode_func(self, t, states, plotting_prob=1e-2):
        ...
        for i in range(1, self.n_grid - 1):
            ...
            drift[i] = (
                ...
                * 0.5
                * (
                    v_hat[i] * (u_t[i] + u_t[i + 1])
                    - v_hat[i - 1] * (u_t[i - 1] + u_t[i])
                )
            )
        ...
        for i in range(1, self.n_grid - 1):
            diffusion[i] = h2inv * (
                D_hat[i - 1] * (u_t[i - 1] - u_t[i]) - D_hat[i] * (u_t[i] - u_t[i + 1])
            )
    ...
    def training_step(self, train_batch, index):
        ...
        with torch.no_grad():
            weight = (
                torch.clamp(torch.log(utp1 + 1e-10), min=-24) + 24
            ) ** self.weight_intensity
            weight /= weight.sum()
        log_utp1_loss = self.loss_fn(
            torch.log(utp1 + 1e-10), torch.log(u_int[-1] + 1e-10), weight=weight
        )
        ...
        boundary_loss = self.density_loss(
            utp1[0], u_int[-1][0], var=u_var_tp1[0]
        ...
        with torch.no_grad():
            ...
            # Log learning rate in scientific notation
            lr = self.optimizers().param_groups[0]["lr"]
            self.log_dict(
                {"learning_rate": float(f"{lr:.2e}")}, on_epoch=True, prog_bar=True
            )
    ...
    def validation_step(self, val_batch, index):
        ...
        with torch.no_grad():
            ...
            self.plot_u_dt(
                ...
                utp1,
                u_int[-1],
                self.g.y.clone().detach().cpu().numpy(),
            )
...
class pde_params_meshgrid(pde_params_base):
    ...
    def ode_func(self, t, states):
        ...
        for dim in range(n_dim):
            ...
            assert (
                v_dim.shape == u_nearby.shape
            ), " u and v[:dim] does not have the same shape"
            ...
            dvu_i_ds = 0.5 * dvuds_dim.sum(
                dim=dim + 1, keepdim=True
            )  # (v_{i+1}u_{i+1} - v_{i-1}u_{i-1})/ 2h

            drift_dim = torch.cat(
                [
                    dvuds_dim.index_select(dim + 1, torch.tensor([0]).to(device)),
                    dvu_i_ds,
                    dvuds_dim.index_select(
                        dim + 1, torch.tensor([square_size - 2]).to(device)
                    ),
                ],
                dim=dim + 1,
            )
            ...
            assert (
                D_nearby.shape == u_nearby.shape
            ), "u and D does not have the same shape"
            u_ss = h_inv**2 * torch.diff(u_nearby, n=2, dim=dim + 1)
            diffusion_dim = torch.mul(
                D_nearby, torch.broadcast_to(u_ss, D_nearby.shape)
            )
...
class pde_neighborloss(pde_params_meshgrid):
    ...
    def training_step(self, train_batch, index):
        ...
        log_utp1_loss = self.loss_fn(
            log_utp1, torch.log(u_int + 1e-10).flatten(), weight=weight
        )
        D_norm = self.restrict_D(s_bund, t_b[:, it].to(device)) + self.restrict_D(
            s_bund, t_b[:, it + 1].to(device)
        )
