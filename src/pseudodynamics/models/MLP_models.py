...
class MLP_surrogate(nn.Module):
    ...
    def __init__(
        self,
        channels: list = [2, 32, 32, 1],
        activation_fn: Union[str, list] = "Mish",
        time_sensitive=True,
    ):
        ...
        if type(activation_fn) == str:
            assert activation_fn in dir(
                nn
            ), "invalid activation function, please check `https://pytorch.org/docs/stable/nn.html`"
            self.act_fns = [activation_fn] * (len(channels) - 1)

        elif type(activation_fn) == str:
            assert (
                len(activation_fn) == len(channels) - 2
            ), "The length of activation_fn should be 2 less than channels"
        ...
        for i, channel in enumerate(in_out):
            ...
            self.u_theta.add_module(
                f"{self.act_fns[i]}_{i}", eval("nn.%s()" % activation_fn)
            )

        self.u_theta.add_module(
            f"Linear_{i+1}", nn.Linear(*channels[-2:])
        )  # output layer
    ...
    def forward(self, s, t) -> torch.Tensor:
        ...
        if self.time_sensitive:
            ...
            if type(t) == int:
                t = torch.full_like(
                    s, fill_value=t, device=s.device, requires_grad=s.requires_grad
                )
            # if t.shape[-1] != 1:
            #     t = t.unsqueeze(-1)
