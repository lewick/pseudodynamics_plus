...
class PINN_base(pl.LightningModule):
    ...
    def trace_div(self, f, s):
        ...
        for i in range(s.shape[1]):
            sum_diag += (
                torch.autograd.grad(f[:, i].sum(), s, create_graph=True)[0]
                .contiguous()[:, i]
                .contiguous()
            )
    ...
    def equation(self, s, t) -> tuple:
        ...
        if len(Du.shape) == 1:  # for one trajectory system
            ...
            for i in range(v.shape[1]):
                du_dsisi = torch.autograd.grad(Du[:, i].sum(), s, create_graph=True)[0][
                    :, i : i + 1
                ]
        ...
        vu = self.mul(v, u)
        dvuds = torch.autograd.grad(vu.sum(), s, create_graph=True)[
            0
        ]  # TODO:check shape
    ...
    def simplified_equation(self, s, t) -> tuple:
        ...
        if len(s.shape) == 1:  # for one trajectory system
            ...
            u_ss_ls = []
            for i in range(v.shape[1]):
                du_dsisi = torch.autograd.grad(duds[:, i].sum(), s, create_graph=True)[
                    0
                ][:, i : i + 1]
    ...
    def population_loss(self, u_pred, Mean, Var) -> torch.Tensor:
        ...
        N_theta = 0.5 * (u_pred[:, 1:] + u_pred[:, :-1]).sum(
            dim=1, keepdim=True
        )  # / h_inv
    ...
    def get_data(self, data_batch, requires_grad=True):
        ...
        t_b = (
            t_b.squeeze(dim=0).float() if len(t_b.shape) == 4 else t_b
        )  # change dimension

        s_all = (
            s_all.squeeze(dim=0).float() if len(s_all.shape) == 4 else s_all
        )  # torch.einsum('ijk->jik', s_all).float()
        ...
        Mean = (
            Mean.squeeze(0).float() if Mean.shape[0] == 1 else Mean.squeeze(-1).float()
        )
        Var = Var.T.float()
    ...
    def predict_param(self, DataSet, param="g"):
        ...
        if (len(param_pred.shape) != 1) and (
            param_pred.shape[1] == DataSet.n_dimension
        ):
...
class PINN_base_sim(PINN_base):
    ...
    def get_data(self, data_batch, requires_grad=True):
        ...
        t_bon = (
            t_bon.squeeze(dim=0).float() if len(t_bon.shape) == 4 else t_bon
        )  # change dimension

        s_bon = (
            s_bon.squeeze(dim=0).float() if len(s_bon.shape) == 4 else s_bon
        )  # torch.einsum('ijk->jik', s_bon).float()
        # if cell state has higher dimension
        # (1, T, n_grid) -> (T, n_grid, 1)
