...
def params_in_umap(
    ...
    if clipping is not None:
        assert (
            len(clipping) == 2
        ), "the format of clipping threshold should be (n_min, n_max)"
    ...
    fig, axs = umap_by_time(
        lambda x: f"Day{x}_{param}",
        adata,
        timepoints,
        time_mask=cell_of_t,
...
def contour_animation(s, continous_u, save_path, fill=False, fps=5):
    ...
    ani = animation.FuncAnimation(
        fig, run, frames=continous_u.shape[0], interval=10, init_func=init
    )  # 製作動畫
    ani.save(save_path, fps=fps, writer="pillow")


def truncated_clustermap(
    matrix,
    num_clusters,
    original_shape=None,
    truncate_mode="level",
    ...
    if original_rows * original_cols != num_columns:
        raise ValueError(
            "original_shape does not match the number of columns in the matrix."
        )
    ...
    cmap_grey = ListedColormap(
        plt.cm.Greys_r(np.linspace(0.15, 0.8, original_cols + 1))
    )
    ...
    bottom_col_ax = g.fig.add_axes(
        [hm_pos.x0, hm_pos.y0 - col_pos.height - 0.005, hm_pos.width, col_pos.height]
    )
    ...
    cax_col_x = g.fig.add_axes(
        [
            col_colorbar_location_x[0],
            col_colorbar_location_x[1],
            col_colorbar_size[0],
            col_colorbar_size[1],
        ]
    )
...
def plot_gene_trends(
    ...
    x = gene_trends_df.columns.astype(
        float
