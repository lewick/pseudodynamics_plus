...
def umap_by_time(
    ...
    default_plotting_kw = dict(
        figsize=(n_timepoints * 2.7, 2), gridspec_kw={"wspace": 0.4}
    )
    ...
    if type(time_mask) == str:
        ...
        timepoint_key = (
            "timepoint_tx_days"
            if "timepoint_tx_days" in anndata.obs_keys()
            else "timepoint"
        )
    for t in timepoints:
        if len(timepoints) == 1:
        ...
        sc.pl.umap(
            anndata, show=False, return_fig=False, ax=ax, alpha=0.5, s=50, frameon=False
        )
...
def plot_along_pseudotime(
    color_col, anndata, pt_col="dpt_pseudotime", timepoints=timepoints
):
    ...
    for t in timepoints:
        ...
        sc.pl.umap(
            ...
            color=col,
            alpha=0.7,
            color_map="viridis",  # colorbar_loc=None,
            return_fig=False,
            show=False,
            ax=axs[0, axis_j],
            s=50,
        ...
        sns.kdeplot(
            anndata.obs.query("`timepoint_tx_days` == @t")[pt_col],
            label="d%d" % t,
            ax=axs[1, axis_j],
        )
...
def scatter_density(color_col, anndata, pt_col="dpt_pseudotime", timepoints=timepoints):
    ...
    for t in timepoints:
        cbs = anndata.obs.query("`timepoint_tx_days` == @t").index
        sns.scatterplot(
            data=anndata.obs.loc[cbs], x=pt_col, y=color_col, ax=axs[axis_j]
        )
        axis_j += 1
    return fig
...
def resampling_animation_meshgrid(s, train_DS, save_path):
    ...
    ani = animation.FuncAnimation(
        fig, run, frames=100, interval=10, init_func=init
    )  # make animation
...
def resampling_animation_umap(train_DS, save_path):
    ...
    ani = animation.FuncAnimation(
        fig, run, frames=200, interval=10, init_func=init
    )  # make animation
    ani.save(save_path, fps=5, writer="pillow")
...
def obs_composition(
    ...
    prop_pivot = cell_type_prop.pivot(
        index=x_var, columns=y_var, values="percentage"
    ).fillna(0)
    ...
    if kind == "area":
        prop_pivot.plot.area(ax=ax, stacked=True, color=colors, linewidth=0)
...
def celltype_proportion(
    ...
    fig_subplot, axs = plt.subplots(
        1, n_timepoint, figsize=(3 * n_timepoint, height), dpi=300, sharey=True
    )
    ...
    for i, t in enumerate(timepoints):
        ...
        sns.barplot(
            data=p_celltype_melt.query("`time` == @t"),
        ...
        for bars, hatch, legend_handle in zip(
            ax.containers, ["", "//"], ax.legend_.legendHandles
        ):
