...
def agg_param(
    ...
    # sanity check
    assert (
        len(timepoints) == param.shape[0]
    ), "timepoints does not match with the given parameter"
    assert (
        adata.shape[0] == param.shape[1]
    ), "cell number does match with the given parameter"
    ...
    # aggregation hre
    agg_param_df = param_df.groupby(groupby_key, observed=False).agg(agg_fn)
    ...
    for it, t in enumerate(timepoints):
        ...
        ct_df.append(
            ad_t.obs.groupby(groupby_key, observed=False).agg({timepoint_key: "count"})
        )

    ct_df = pd.concat(ct_df, axis=1)
    ct_count_thres_binary = (
        ct_df.where(ct_df >= cellcount_threshold, np.nan) / ct_df.values
    )
...
def continuous_params(
    ...
    for it, s_ts in tqdm(enumerate(DataSet.cellstate_t_ls[:-1])):
        ...
        if groupby_key is not None:
            ...
            obs_t["str_col"] = "x"
            ct_count_dict = (
                obs_t.query(f"`{DataSet.timepoint_key}` == @real_time")
                .groupby([groupby_key], observed=False)
                .agg({"str_col": "count"})
                .to_dict()["str_col"]
            )
        ...
        for t_mid in int_timepoint:
            ...
            for i in range(0, len(t_ts), chunk_size):
                ...
                # forward here
                param_pred = (
                    pde_model.__getattr__(param)(s_in, t_in)
                    / pde_model.time_scale_factor
                )
                if param == "v":
                    # get l2 norm of this
                    param_pred = torch.norm(param_pred, dim=1)
        ...
        if groupby_key is not None:
            params = (
                obs_t[agg_col + [groupby_key]]
                .groupby(groupby_key, observed=False)
                .agg(agg_fun)
            )
            params = pd.melt(
                params.reset_index(), id_vars=groupby_key, value_name=param
            )
...
def density_shortterm_simulation(
    pde_model,
    DataSet,
    timepoint_idx=None,
    time_span=1,
    ...
    for it, itp1 in tqdm(zip(timepoint_idx[:-1], timepoint_idx[1:])):
        ...
        for i in range(0, len(cellstate), chunk_size):
            ...
            tompos_u0 = (
                torch.from_numpy(u_b[it, i : i + chunk_size])
                .float()
                .to(device)
                .requires_grad_()
            )
            duds_0 = (
                torch.from_numpy(duds[it, i : i + chunk_size, :])
                .float()
                .to(device)
                .requires_grad_()
            )
        ...
        for i, ith_out in enumerate(out_t):
            conate_axis = (
                1 if len(ith_out[0].shape) > 0 else 0
            )  # 1 is sample wise if more than 1 evaluation timepoint
    ...
    all_output = [
        np.concatenate([o[i] for o in all_output], axis=0) for i in range(len(out_t))
    ]
...
def param_vs_score(
    adata, obs_key, param, timepoints=None, timepoint_key="timepoint_tx_days"
):
    if timepoints is None:
    ...
    assert (
        len(timepoints) == param.shape[0]
    ), "the provided timepoints and the given params must the same dimension"
    spr = []
    pr = []
...
def project_params_to_pseudotime(
    adata,
    params,
    param_names="g v2",
    timepoints=None,
    ...
    adata.obs["pseudotime_bin"] = pd.cut(
        adata.obs[pseudotime_key], bins=nbins, labels=pdt_label
    )
    ...
    for i, t in enumerate(timepoints):
        ...
        data[y_col] = params[i]
        gdata = (
            data.groupby("pseudotime_bin", observed=False)
            .agg({y_col: "mean"})
            .reset_index()
        )
...
def assign_nearest_cell(
    ...
    assert (
        input_ay.shape[1] == cellstate.shape[1]
    ), "the dimension of cellstate and the query don't match"
    ...
    if annotation is not None:
        nn_return = pd.DataFrame(
            adata[nn.flatten()].obs[annotation].to_numpy().reshape(-1, n_neighbors)
        ).T
...
def KLD_density(u_b, u_simulate, sanity_check=True):
    ...
    if sanity_check:
        assert (
            u_b_local.shape == u_simulate_local.shape
        ), "observation and prediction must be the same"
