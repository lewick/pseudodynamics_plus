...
optional_args.add_argument(
    ...
    required=False,
    default=None,
    help="in what name to save the model",
)
optional_args.add_argument(
    "--progress_bar",
    type=str,
...
if args.model_note is not None:
    save_path = os.path.join(save_path, args.model_note)
...
t_pred = mellon.Predictor.from_json(
    f"data/{args.dataset}_mellon_timecontinuous_predictor.json"
)
...
DS_fun = lambda ad: reader.TwoTimpepoint_AnnDS(
    AnnData=ad,
...
train_DS = DS_fun(adata[adata.obs.split == "train"])
val_DS = DS_fun(adata[adata.obs.split == "val"])
test_DS = DS_fun(adata[adata.obs.split == "test"])

train_DL = DataLoader(train_DS, batch_size=None, num_workers=10)
val_DL = DataLoader(val_DS, batch_size=None, num_workers=10)
test_DL = DataLoader(test_DS, batch_size=None, num_workers=10)
...
config_save_path = os.path.join(save_path, f"V{version}_config.json")


config_run = pseudodynamics.ExperimentConfig(args=args, model=model)
config_run.experiment_config["save_dir"] = save_path
config_run.experiment_config["dataset_class"] = train_DS.__class__.__name__
config_run.experiment_config["model_class"] = model.__class__.__name__
config_run.experiment_config["version"] = version
config_run.experiment_config["checkpoint_dir"] = trainer.logger.log_dir
config_run.save(config_save_path)
