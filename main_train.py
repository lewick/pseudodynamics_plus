...
parser = argparse.ArgumentParser(
    "Training PINN dynamics on mesh-free high dimensional cellstate"
)
parser.add_argument(
    "--config",
    type=str,
...
optional_args = parser.add_argument_group(
    "Optional arguments (ignored when using --config)"
)
optional_args.add_argument(
    ...
    required=False,
    default="HSPC_clu7",
    help="the name of the dataset, can be found under folder data",
)
optional_args.add_argument(
    "-K",
    "--cellstate_key",
    ...
    required=False,
    default="cellstate",
    help="the obsm key on which we represent cell and compute density",
)
optional_args.add_argument(
    "-M",
    "--model",
    ...
    required=False,
    default="pde_params",
    help="the model class, defined in models.py",
)
optional_args.add_argument(
    "-W",
    "--pretrained",
...
optional_args.add_argument(
    ...
    required=False,
    default=None,
    help="select which gpu devices to use",
)
optional_args.add_argument(
    "-L",
    "--log_name",
    ...
    required=False,
    default=None,
    help="the name of the logging directory",
)
optional_args.add_argument(
    "--lr",
    type=float,
...
optional_args.add_argument(
    ...
    required=False,
    default="CyclicLR",
    help="LambdaLR if passing a lambda expression, else StepLR",
)
optional_args.add_argument(
    "--n_grid",
    type=int,
    required=False,
    default=300,
    help="the number of grid or h to devid the cell state space",
)
optional_args.add_argument(
    "--n_dimension",
    type=int,
    required=False,
    default=5,
    help="the number of dimension to used for estimating density",
)
optional_args.add_argument(
    "--timepoint_idx",
    type=str,
    required=False,
    default=None,
    help="the number of time point to train the model",
)
optional_args.add_argument(
    "--knn_volume",
    type=str,
    required=False,
    default=False,
    help="Whether to correct single-cell density estimate with KNN distance",
)
optional_args.add_argument(
    "--batch_size",
    type=int,
    required=False,
    default=200,
    help="the number of nearby cell state to include within a minibatch",
)
optional_args.add_argument(
    "--bw",
    type=float,
    required=False,
    default=None,
    help="the band width parameter , pass to bw_method for gaussian_kde",
)
optional_args.add_argument(
    "--tol",
    type=float,
    required=False,
    default=1e-4,
    help="the tolerance of error , used to control the precision and speed of ode integral",
)
optional_args.add_argument(
    "--channels",
    type=str,
    required=False,
    default="32,32",
    help="the depth and width of the hidden layers",
)
optional_args.add_argument(
    "--D_penalty",
    type=float,
    required=False,
    default=None,
    help="the weight to regulate the level of D (Diffusion)",
)
optional_args.add_argument(
    "--deltax_key",
    type=str,
    required=False,
    default="Delta_DM",
    help="the key to take deltax from adata",
)
optional_args.add_argument(
    "--deltax_weight",
    type=float,
    required=False,
    default=1e-2,
    help="the weight used to regularize the similarity of deltax and v",
)
optional_args.add_argument(
    "--weight_intensity",
    type=float,
    required=False,
    default=None,
    help="the weight to emphasize the high density cell, > 1 for weighting, <1 for unweighting",
)
optional_args.add_argument(
    "--R_weight",
    type=float,
    required=False,
    default=None,
    help="the weight to balance PDE residue loss and the data-related loss",
)
optional_args.add_argument(
    "--growth_weight",
    type=float,
    required=False,
    default=None,
    help="the weight to regularize the contribution of growth to overall density gain, greater means harder boundary",
)
optional_args.add_argument(
    "--time_scale_factor",
    type=float,
    required=False,
    default=5,
    help="the scale the time for ode",
)
optional_args.add_argument(
    "--norm_time",
    type=str,
    required=False,
    default=False,
    help="Ways to normlize the timepoint, [False, min_minus, log, none]",
)
optional_args.add_argument(
    "--time_sensitive",
    action="store_true",
    required=False,
    help="Whether to include time in behavoir functions",
)
optional_args.add_argument(
    "--progress_bar",
    type=str,
    required=False,
    default="True",
    help="whether show progress bar on screen, boolen value, default True",
)
optional_args.add_argument(
    "--max_epochs",
    type=int,
...
if args.config:
    ...
    if args.gpu_devices is None:
        parser.error(
            "The following arguments are required without --config: -G/--gpu_devices"
        )
...
path = path + "/trafficking_inference/proliferation_stuff/models"
...
if args.timepoint_idx is None:
    ...
    args.timepoint_idx = (
        eval(args.timepoint_idx)
        if isinstance(args.timepoint_idx, str)
        else args.timepoint_idx
    )
...
save_path = os.path.join(
    main_path, "logs", log_name, args.model + ["", "_tsense"][args.time_sensitive]
)
pseudodynamics.tl.make_dir(save_path)
print(f"Model checkpoints and logs will be saved to: {save_path}")
...
if args.pretrained is not None:
    ...
    if Pretrain_class == args.model:
        ...
        Pretain_model = Pretrain_class.load_from_checkpoint(
...
train_DL = DataLoader(train_DS, batch_size=None, num_workers=16)
val_DL = DataLoader(val_DS, batch_size=None, num_workers=16)
##############################
##      set up trainer      ##
##############################
...
print(device)
gpu_device = 4 if (args.gpu_devices == "None") else [int(args.gpu_devices)]
...
trainer = pl.Trainer(
    ...
    devices=gpu_device,
    logger=pl_loggers.CSVLogger(save_dir=save_path),
    max_epochs=args.max_epochs,
    log_every_n_steps=int(args.batch_size // adata.n_obs),
...
# Plot metrics over time using CSV logger
metrics_df = pd.read_csv(os.path.join(trainer.logger.log_dir, "metrics.csv"))
